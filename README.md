...
**Current status.** The build arc is complete. Works today: interactive chat via `streaming/chat.py` at ~1–2 tok/s for short-form use, at full mixed 4/8-bit quality, under 24 GB peak memory, with clean prose output through DeepSeek's official chat encoding (see *Chat usage*); a local OpenAI-compatible server, `streaming/serve.py` (see *Server*); and a perplexity run through the streaming path whose partial-corpus result is consistent with numerical transparency (see *Perplexity through the streaming path*). What remains open are the parked research questions: decode drops to ~0.25–0.38 tok/s after a long prefill, the obvious explanation has been tested and rejected (see *Batched prefill*), and the server's repeated-request regression shares the same root cause; and the directly comparable full-corpus perplexity number is an unrun ~17 h job.
...
- **Perplexity validation is partial.** Decoding is byte-correct against the reference implementation and output text was byte-identical across all 19 sweep runs; the streaming-path perplexity run (*Perplexity through the streaming path*) covers windows 1–8 of wikitext-2 and is consistent with numerical transparency, but the directly comparable number requires all 195 windows (published: 6.1262) — a ~17 h run that has not been done. Chat output is additionally validated at the encoding level: all 4 bundled `encoding_dsv4` test vectors pass byte-exact.
