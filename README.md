...
**Current status.** Works today: interactive chat via `streaming/chat.py` at ~1–2 tok/s for short-form use, at full mixed 4/8-bit quality, under 24 GB peak memory, with clean prose output through DeepSeek's official chat encoding (see *Chat usage*). Open: decode drops to ~0.25–0.38 tok/s after a long prefill, and the obvious explanation has been tested and rejected (see *Batched prefill*); perplexity validation is in progress; a server wrapper is pending.
...
- **Output quality is unvalidated.** Decoding is byte-correct against the reference implementation and output text was byte-identical across all 19 sweep runs, but a perplexity validation through the streaming path (target: match pipenetwork's 6.13 for mixed 4/8-bit) is still in progress. Chat output is additionally validated at the encoding level: all 4 bundled `encoding_dsv4` test vectors pass byte-exact.
