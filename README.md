...
- Exclude the repacked directory from Spotlight — `touch repacked/.metadata_never_index` — before the first run. This is done in this repo. See the stall discussion below for why it is suspected to matter; the effect has not been measured.
...
Two things still qualify the comparison. The two rows are not equal-quality: the baseline is a 1.58-bit quant, so the streaming path is doing more arithmetic per token for a better output distribution, and no perplexity check has been run through the streaming path to quantify that. And the streaming figure is one prompt at 24 tokens; llama.cpp CPU-only holds every weight resident and has no cache dependence at all, so its number should hold across prompts while this one is a workload-sensitive measurement that has been characterized on one workload.
...
So the extreme stalls are a **sporadic system-level I/O phenomenon that can strike any cache size**, and cache size does not predict them. What cache size does predict is the ordinary, reproducible ~1.7x spread in the medians.

The stalls' actual cause is not known. One untested hypothesis: Spotlight was indexing `repacked/`, 165 GB of large binary files being read continuously, and an indexing pass contending for the same I/O path would produce exactly this shape — long, sporadic, unrelated to our own memory footprint. `repacked/.metadata_never_index` has now been added to exclude the directory, but no controlled before/after measurement has been run, so this remains a suspicion, not a finding. Also note that `pass1-fwd` was uniformly slow at *every* budget including 4 GB (1.61 tok/s where later passes gave 1.97–2.01), which is consistent with a whole-machine state effect rather than anything cache-size-specific.

**The methodological point survives intact, and is stronger than before:** cache size must be chosen by measured end-to-end throughput, never by hit rate, and never by how much the wired limit will nominally permit. It just needs more replicates than we originally gave it, because single runs on this machine are capable of being 62x off.
...
48 new tokens each. The two diverse prompts ran back to back in one process with a 40 GB cache budget, saturated (`cache 40.0 GB` on both lines); the repetitive row is the 44 GB run from the table above, repeated here for comparison. **Read the throughput of these runs as void:** 40 GB is now known to be a bad cache size, and the same factual prompt runs at 2.04 tok/s at 8 GB. What this section still establishes is the *structure* of the workload — distinct experts, bytes read, and where the reuse comes from — which is independent of cache sizing.
...
**The working set that fit was a property of the prompt.** A realistic prompt touches ~4,050 distinct experts, ~57 GB — more than the largest cache that fits on 64 GB hardware (~44 GB, and only with a raised wired limit). The LRU therefore churns, and each new prompt re-pays most of its cold cost. "Zero new misses on the warm pass" describes `"The capital of X is Y"` repeated forever, not inference in general. This was written as bad news, on the assumption that chasing the working set with a bigger cache was the goal; the sweep says the chase was the mistake. Since the working set cannot fit anyway, the right move is to stop competing with the OS for memory and let the page cache do the caching.
...
**The SSD is not what the extra misses cost.** 57 GB of misses at the measured 13.3 GB/s is ~4.3 s of I/O per 48-token generation, which would be affordable. The cost lands on the memory system instead — see the sweep above, which localizes it to the interaction between the MLX-side cache and the OS page cache rather than to the SSD.

#### The "swap cliff" (superseded)

This subsection previously argued that a 40 GB cache tips the machine into swap and that this is what produced a 17x wall-clock difference (55.0 s vs 928.8 s) on byte-identical work. The cache-size sweep tested that directly and only half of it held up. See *Cache size, swap, and sporadic stalls* above for the corrected version: large caches are reliably slower and do ratchet swap upward, but the extreme stalls are sporadic, occur at small cache sizes too, and are not explained by cache size. The transcribed evidence is preserved in `docs/evidence-swap-cliff.md` with the same correction noted.
...
- **The extreme stalls are unexplained.** A 62x collapse hit an 8 GB run mid-sweep and was not reproducible. The Spotlight hypothesis is untested. Any single timing measurement on this machine should be assumed capable of being wildly wrong until replicated.
- **The diverse-prompt evidence is 2 prompts, not 8 — this is partial data.** `multi_prompt_eval.py` defines 8 domains; the run was interrupted after `factual` and `coding` completed, so there is no cross-domain spread, no per-domain variance, and no summary JSON. Enough to characterize the working set, not the workload. The remaining six prompts have not been run — and they should now be re-run at 8 GB, since the existing pair used the discredited 40 GB budget.
- **Output quality is unvalidated.** Decoding is byte-correct against the reference implementation and output text was byte-identical across all 19 sweep runs, but a perplexity validation through the streaming path (target: match pipenetwork's 6.13 for mixed 4/8-bit) is still pending.
...
- **Wired-limit fragility — now avoidable.** Cache budgets above ~40 GB depend on `iogpu.wired_limit_mb`, which resets on reboot. The recommended 8 GB configuration does not touch it, so this is no longer a limitation of the normal path, only of the large-cache experiments. Caveat: the entire sweep, including the 4 and 8 GB runs, was executed on a machine with the raised limit in effect, so "the sysctl is unnecessary at 8 GB" is an inference from the 17.7 GB peak rather than something that was tested with the limit at its default.
